...
A unidade de controle segue então um ciclo de execução durante todo o tempo em que está em funcionamento: primeiro ela faz a **busca**, que é uma operação de leitura na memória, no endereço contido no PC, para obter o código da instrução a executar.
...
Inicialmente, deve conseguir a identificação do dispositivo que está em PC+1 -- PC vale 0, PC+1 é 1, e a UC pede para a memória o valor contido nesse endereço (obtém 2).
Então, faz uma operação de leitura de entrada e saída, passando esse valor 2.
