...
Esse programa já tá ficando complicade demais para ficar em uma só função:
```c
#include <stdio.h>
#include <stdlib.h>
#include <time.h>
...
void le_uma_jogada(int m[3][3])
{
  ...
  while (true) {
    scanf("%d%d", &lin, &col);
    if (lin >= 1 && lin <= 3 && col >= 1 && col <= 3 && m[lin][col] == 0) {
      break;
    }
...
1. Complete as funções que faltam no programa acima, para ter um programa que joga o jogo da velha. A função que faz uma jogada pode sortear linha e coluna até cair em uma posição vazia. Depois que o programa estiver completo, pense em como poderia fazer escolhas melhores.
